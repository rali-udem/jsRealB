...
function initial_prompt (){
    return ask(choose(elizaInitials,[]));
}

function want_to_quit(userInput){
    return elizaQuits.includes(userInput.toLowerCase()) ? "yes" : "no"
}
                            
function bye(){
    return ask(choose(elizaFinals),[])
}
...
//const readline = require('node:readline/promises'); // Import standard callback module
...
//        if (want_to_quit(userInput)){// dernière question
//            console.log(bye()) 
//            rl.close()
//            break;
//        }
