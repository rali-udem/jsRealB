...
function parseUDs(groupVal,fileName){
    ...
    udsStrings.forEach(function(string){
        ...
        const ud=new UD(language || getLanguage(),fileName,string,startLine);
        // console.log(ud.show());
...
function parse(udContent,fileName){
    ...
    uds.forEach(function (ud,i){
        ...
        ud.diffs=computeDiffs(ud.text,ud.jsRealBsent,language=="en");
...
function updateRealization(){
    ...
    currentUD.diffs=computeDiffs(currentUD.text,realization,language=="en");
    showRealization(currentUD,false);
}
