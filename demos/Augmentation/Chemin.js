...
function Chemin(actions){
    this.actions=actions;
}

// ajouter une nouvelle action ainsi que la direction si c'est un test
Chemin.prototype.ajouter = function(etat,direction){
...
Chemin.prototype.ajouterHandlers = function(){
    const chemin=this;
    function mouseOver(scroll,i){
        ...
        if(scroll)scrollIfNeeded(i,chemin);
    }
    ...
    for (let i=0;i<n;i++){
        ...
        if (!(action instanceof Etape))action=action[0];
        d3.select("#p"+i)
...
Chemin.prototype.enleverHandlers = function(){
    const n=this.actions.length;
    for (let i=0;i<n;i++){
...
Chemin.prototype.montrer = function(){
...
Chemin.prototype.allonger =  function(){
    const monChemin=this;
    function handle(choix,next){ // attention this ici n'est plus le chemin courant
...
function scrollIfNeeded(i,chemin){
