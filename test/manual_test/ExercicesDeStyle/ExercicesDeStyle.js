...
var lexique,lexicon; // variables globales utiles pour tester sur la console Javascript

function completerLexique(){
    lexique=JSrealB.Config.get("lexicon");
    lexique["narrateur"] = { "N": { "g": "m", "tab": ["n56"]} }; 
    lexique["bus"]       = { "N": { "g": "m", "tab": ["n2"] } };
    lexique["bondé"]     = { "A": { "g": "m", "tab": ["n28"] } };
    lexique["coiffé"]    = { "A": { "g": "m", "tab": ["n28"] } };// car le participe passé ne s'accorde pas!
    lexique["quelque"]   = { "D": { "g": "m", "tab": ["n28"] } };
}
...
function composer(t,g,n){
  ...
  ps[3]=
  S(SP(NP(NO(2),N("heure")),AdvP(Adv("plus"),Adv("tard"))).a(","),
    narrateur,
  ...
  S(NP(Pro("celui-ci").g(g).n(n).tag("span", {"class": "ami genre nombre"})),
    n==="s"?"lui":"leur",  // !!!patch car 'Pro("me").g(g).n(n).tag("span", {"class": "genre nombre jeune-homme"})'  
                        // utilise la table pn2 au lieu de pn3 car non prise en compte du type du pronom
...
function updateLexicon(){
    lexicon=JSrealB.Config.get("lexicon");
    lexicon["gal"] = {"N":{"tab":["n1"]}};
    lexicon["every"] = {"D":{"tab":["d4"]}};
}
...
function compose(t,g,n){
    ...
    friend=N("friend").n(n).tag("span", {"class": "ami nombre"});
    var ps=[];
    ...
    S(
      SP(C("when"),
        VP(chapProSuj,
            ...
            NP(D("a"),A("vacant"),N("seat").n(n)).tag("span", {"class": "nombre"})),
            VP(chapProSuj,
    ...
    // 2 hours later, I come across him in front of the Gare Saint-Lazare.
    ps[4]=
    S(NP(NO(2),N("hour"),A("late").f("co")).a(","),
      VP(speakerPro,
...
function rediger(e){
    loadLanguage("../../../data/","fr",function(){
        completerLexique();  
        texte.html("")
        texte.append(composer(temps.val(),genre.val(),nombre.val()));
    });
    loadLanguage("../../../data/","en",function(){
        updateLexicon();
        text.html("")
        text.append(compose(temps.val(),genre.val(),nombre.val()));
    });
